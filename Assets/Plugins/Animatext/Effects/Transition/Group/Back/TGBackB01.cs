...
{
    [CreateAssetMenu(menuName = "Animatext Preset/Transition - Group/Back/Back - B01", fileName = "New TGBackB01 Preset", order = 369)]
    public sealed class TGBackB01 : DefaultTemplateEffect
    {
        ...
        protected override void Animate()
        {
            for (int i = 0; i < groupCount; i++)
            {
                ...
                else
                {
                    ...
                    float progressA = EasingUtility.EaseBack(progress, amplitude);
                    Vector2 anchorPoint = groups[i].GetAnchorPoint(anchorType) + anchorOffset;

                    groups[i].Scale(Vector2.LerpUnclamped(Vector2.one, scale, progressA), anchorPoint);
                    groups[i].Rotate(rotation * progressA, anchorPoint);
                    groups[i].Move(positionA * progressA + positionB * progress);
                }
            }
        }
    }
