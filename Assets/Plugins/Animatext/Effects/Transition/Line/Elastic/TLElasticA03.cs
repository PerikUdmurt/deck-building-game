...
{
    [CreateAssetMenu(menuName = "Animatext Preset/Transition - Line/Elastic/Elastic - A03", fileName = "New TLElasticA03 Preset", order = 369)]
    public sealed class TLElasticA03 : DefaultTemplateEffect
    {
        ...
        protected override void Animate()
        {
            for (int i = 0; i < lineCount; i++)
            {
                ...
                else
                {
                    ...
                    lines[i].Scale(Vector2.LerpUnclamped(Vector2.one, scale, progress), lines[i].GetAnchorPoint(anchorType) + anchorOffset);
                }
            }
        }
    }
