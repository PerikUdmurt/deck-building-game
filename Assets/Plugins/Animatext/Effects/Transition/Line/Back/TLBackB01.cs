...
{
    [CreateAssetMenu(menuName = "Animatext Preset/Transition - Line/Back/Back - B01", fileName = "New TLBackB01 Preset", order = 369)]
    public sealed class TLBackB01 : DefaultTemplateEffect
    {
        ...
        protected override void Animate()
        {
            for (int i = 0; i < lineCount; i++)
            {
                ...
                else
                {
                    ...
                    lines[i].Scale(Vector2.LerpUnclamped(Vector2.one, scale, progressA), anchorPoint);
                    lines[i].Rotate(rotation * progressA, anchorPoint);
